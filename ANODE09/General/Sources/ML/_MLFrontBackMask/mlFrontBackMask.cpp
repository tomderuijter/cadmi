...
ML_MODULE_CLASS_SOURCE(FrontBackMask, Module);

//----------------------------------------------------------------------------------

FrontBackMask::FrontBackMask() : Module(1, 1)
{
...
void FrontBackMask::handleNotification(Field* field)
{
...
void FrontBackMask::calculateOutputImageProperties(int /*outputIndex*/, PagedImage* outputImage)
{
  // Setup data types and read-only flags of output image and input sub-images.
  FrontBackMaskOutputImageHandler::setupKnownProperties(outputImage);
  ...
  FrontBackMaskOutputImageHandler::verifyProperties(outputImage);
}

//----------------------------------------------------------------------------------

CalculateOutputImageHandler* FrontBackMask::createCalculateOutputImageHandler(PagedImage* outputImage)
{
  FrontBackMaskOutputImageHandler::Parameters processingParameters;
  return new FrontBackMaskOutputImageHandler(outputImage->getOutputIndex(), processingParameters);  
}
